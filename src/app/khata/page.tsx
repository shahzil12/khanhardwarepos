          ...
          <table className="w-full text-left border-collapse text-xs sm:text-sm">
            ...
            <tbody className={`divide-y ${isDark ? 'divide-slate-800/60' : 'divide-slate-200'}`}>
              {filteredKhatas.length === 0 ? (
                <tr>
                  <td colSpan={6} className={`p-8 text-center ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>No Khata customer accounts found.</td>
                </tr>
              ) : (
                filteredKhatas.map((khata) => (
                  <tr key={khata.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 font-medium">
                    <td className="p-4 font-bold">
                      <p className={isDark ? 'text-slate-100' : 'text-slate-900'}>{khata.customerName}</p>
                      {khata.cnic && <span className={`block text-[10px] font-mono ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>CNIC: {khata.cnic}</span>}
                    </td>
                    <td className={`p-4 font-bold ${isDark ? 'text-slate-300' : 'text-slate-800'}`}>{khata.phone}</td>
                    <td className={`p-4 font-bold ${isDark ? 'text-slate-300' : 'text-slate-800'}`}>{formatCurrency(khata.creditLimit)}</td>
                    <td className="p-4">
                    ...
                    <td className={`p-4 font-semibold text-xs ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{new Date(khata.updatedAt).toLocaleDateString()}</td>
